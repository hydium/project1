...
thread_init (void) 
{
  ...
  /* Initialization of a list of waiting threads. */
...
thread_tick (void) 
{
  ...
  /* Check the thread waiting list for threads which should be woken up now, and
    if it is the case, wake them up. */

  struct list_elem *e = list_head (&thread_list);
  while ((e = list_next (e)) != list_end (&thread_list)) 
    {
      struct thread_waiting *tw = list_entry (e, struct thread_waiting, elem);
      tw->time_to_wake_up--;
      if (tw->time_to_wake_up <= 0) 
        {
          list_remove(e);
          thread_unblock(tw->thread_waiting_pointer);
        }
    }
...
{ 
  ...
  /* If donation is present, wait for it to finish first and then set the priority.
     Otherwise, set priority right away. */
  if (running_thread()->priority == running_thread()->original_priority)
    {
      running_thread()->priority = new_priority;
      running_thread()->original_priority = new_priority;
    }
  else
    thread_current ()->original_priority = new_priority;

  /* If the current thread does not have the highest priority anymore, yield. The thread with the 
  highest priority in ready list is the first one, due to the way threads are pushed into ready list. */
